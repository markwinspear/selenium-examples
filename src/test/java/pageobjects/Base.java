...
public class Base implements Config{
    ...
    /**
     * Navigate to the URL specified and maximise the browser window (if not mobile as determined by the host parameter)
    ...
    /**
     * Wrapper to return a WebElement based on the locator
    ...
    public Boolean optionsExistInSelectList(By locator, String[] optionsToFind) {
        ...
        try {
            ...
            for (WebElement item:options) {
                for (String anOptionToFind : optionsToFind) {
                    if (item.getText().equals(anOptionToFind)) {
                        match = true;
                    }
                }
            }
